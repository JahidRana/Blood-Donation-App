...
public class MapsMarkerActivity extends FragmentActivity implements OnMapReadyCallback {
    ...
    public void onMapReady(GoogleMap googleMap) {
        mMap = googleMap;
        List<DomainUserInfo> list=AllUserInfoList_Activity.mapL;
        for(int i=0;i<list.size();i++){
            if(list.get(i).latitude !=null&&list.get(i).longitude !=null)
            {
                double lat=Double.parseDouble(list.get(i).latitude);
                double lon=Double.parseDouble(list.get(i).longitude);
                addMarker(lat,lon);
            }
